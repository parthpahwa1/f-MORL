...
class ReplayMemory:
    ...
    def push(self, state, preference, action, reward, next_state, next_preference, done, agent):
        ...
        hq_0 = agent.critic_target(torch.FloatTensor(next_state.reshape(1,-1)), next_preference.reshape(1,-1), torch.FloatTensor(np.array([[0.]])))[0]
        hq_1 = agent.critic_target(torch.FloatTensor(next_state.reshape(1,-1)), next_preference.reshape(1,-1), torch.FloatTensor(np.array([[1.]])))[0]
